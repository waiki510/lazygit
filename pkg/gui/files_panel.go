...
type pushWithForceFlagOpts struct {
	force          bool
...
func (gui *Gui) pushWithForceFlag(opts pushWithForceFlagOpts) error {
		...
		if err != nil && !opts.force && strings.Contains(err.Error(), "Updates were rejected") {
			...
			_ = gui.ask(askOpts{
				...
				handleConfirm: func() error {
					...
					return gui.pushWithForceFlag(newOpts)
				},
			})
			return
		}
...
func (gui *Gui) pushFiles() error {
	...
	if currentBranch.IsTrackingRemote() {
		if currentBranch.HasCommitsToPull() {
			...
			return gui.pushWithForceFlag(pushWithForceFlagOpts{})
		}
	} else {
		...
		if gui.GitCommand.PushToCurrent {
			return gui.pushWithForceFlag(pushWithForceFlagOpts{setUpstream: true})
		} else {
			return gui.prompt(promptOpts{
				title:          gui.Tr.EnterUpstream,
				initialContent: "origin " + currentBranch.Name,
...
func (gui *Gui) requestToForcePush() error {
	...
	return gui.ask(askOpts{
		...
		prompt: gui.Tr.ForcePushPrompt,
		handleConfirm: func() error {
			return gui.pushWithForceFlag(pushWithForceFlagOpts{force: true})
		},
	})
}
